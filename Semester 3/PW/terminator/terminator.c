...
void handler(int signal_number)
{
    switch (signal_number) {
       // case SIGCHLD:
        //    const char st[] = "Parent: Child has finished. Exiting the loop. \n";
...
int parent(pid_t child_pid)
{
    ...
    while (true) {
        int s = sigwaitinfo(&signals_to_wait_for, &info);
        if (s == SIGINT) {
            ...
            ASSERT_SYS_OK(kill(child_pid, SIGINT));
            int ret = sigtimedwait_seconds(&signals_to_wait_for, &info, 1.0);
            if (ret == -1) // Timeout.
                continue;
            else {
                printf("Parent: got second signal >>%s<< from %d\n", strsignal(info.si_signo), info.si_pid);
                ASSERT_SYS_OK(kill(child_pid, SIGKILL));
                break;
            }
        }
        else if (s == SIGCHLD) {
            printf("Parent: Child has finished. Exiting the loop.\n");
            break;
        }
