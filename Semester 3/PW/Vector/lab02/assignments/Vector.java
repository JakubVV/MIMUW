...
public class Vector {
    ...
    public Vector sum(Vector v) throws InterruptedException {
        ...
        int chunks = Math.ceilDiv(elements.length, SUM_CHUNK_LENGTH);
        for (int i = 0; i < chunks; i++) {
            if (i < chunks - 1)
                threads.add(new Thread(new SumHelper(this, v, result, i * SUM_CHUNK_LENGTH, (i + 1) * SUM_CHUNK_LENGTH)));
            else
                threads.add(new Thread(new SumHelper(this, v, result, i * SUM_CHUNK_LENGTH, result.elements.length)));
        }
    ...
    public int dot(Vector v) throws InterruptedException {
        ...
        int nThreads = Math.ceilDiv(elements.length, DOT_CHUNK_LENGTH);
        ...
        for (int i = 0; i < nThreads; i++) {
            if (i < nThreads - 1)
                threads.add(new Thread(new DotHelper(this, v, i * DOT_CHUNK_LENGTH, (i + 1) * DOT_CHUNK_LENGTH, partialResults, i)));
            else
                threads.add(new Thread(new DotHelper(this, v, i * DOT_CHUNK_LENGTH, v.elements.length, partialResults, i)));
        }
        for (Thread thread : threads) {
            thread.start();
        }
